...
1. **cosine distance 임계값을 0.30 → 0.25 로 좁혀 우회 마진을 줄인다** — **적용 완료
   (2026-08-07)**. 이후 실측에서 본인 얼굴 매칭이 **0.025** 로 나와, 기존 폰 화면 재생
   공격값 0.3049 와의 간격이 충분히 넓음을 확인했다. 0.25 로 내려도 정상 인식 여유가
   크다 (`lib/vectorMath.js`, 회귀 테스트에 두 실측값을 고정).
...
3. **jitter 지표를 라이브니스 근거에서 제외** — **적용 완료 (2026-08-07)**. 다만 당초 대안으로
   적었던 "눈깜빡임 필수화" 는 채택하지 않았다. 스캔 구간이 1.5초(14프레임 × 110ms)로 평균
   깜빡임 간격(3~4초)보다 짧아 실측에서 `blinkDetected` 가 대부분 `false` 이고, 필수 조건으로
   걸면 정상 사용자가 통과하지 못하기 때문이다. 대신 **라이브니스 근거를 §5 능동 챌린지 통과로
   옮겼다** — 챌린지에 실패하면 프레임 캡처 단계에 도달하지 못하므로, `livenessPassed` 는
   "지시받은 랜덤 시퀀스를 수행했음" 을 뜻한다. `blinkDetected` / `jitterScore` 는 판정에서
   빠지고 진단용 기록으로만 남는다 (대시보드·리드아웃에도 "진단용" 으로 표기).

> 남은 과제: 광원 변화 감지 등 **카메라 앞 물체가 3D 인지** 를 보는 별도 지표. 현재 챌린지는
> 2D 랜드마크 기반이라 평면 화면을 물리적으로 기울인 경우와 실제 얼굴 회전을 구분하지 못한다
> (§5-3 참고).
