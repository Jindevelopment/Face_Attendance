...
testCase("FACENET512_COSINE_THRESHOLD 값(실측 재보정: 0.25)", () => {
  assert.equal(FACENET512_COSINE_THRESHOLD, 0.25);
});

// 실측 분포가 임계값 양쪽으로 충분히 벌어져 있는지 고정한다 (README §4-2).
// 본인 매칭 0.025 는 통과하고, 폰 화면 재생 공격 0.3049 는 차단돼야 한다.
testCase("실측 샘플이 임계값 기준으로 올바르게 갈린다", () => {
  assert.ok(0.025 < FACENET512_COSINE_THRESHOLD, "본인 얼굴(0.025)은 통과해야 함");
  assert.ok(0.3049 > FACENET512_COSINE_THRESHOLD, "폰 화면 재생(0.3049)은 차단돼야 함");
});
