...
// yaw(좌우 회전) 정규화 근사값. face-api.js 68 랜드마크의 코 끝(30)과 좌/우 턱선 끝(0, 16)
// x 좌표로 계산. 정면일 때 코 끝이 좌우 턱선의 중앙 근처에 위치한다는 성질을 이용.
//
// 좌표 기준(중요): face-api.js 는 raw 비디오 프레임 좌표를 반환한다. CSS 의 scaleX(-1)
// 미러는 표시용일 뿐 랜드마크 좌표에는 영향 없음. 정면 카메라에서 landmarks[0] 는
// 화면 왼쪽(=사용자 본인의 오른쪽 얼굴), landmarks[16] 는 화면 오른쪽(=사용자 본인의 왼쪽).
// 사용자가 본인 기준 왼쪽으로 고개를 돌리면 raw 프레임에서 코가 오른쪽(x 증가)으로 이동.
//
// 반환값:
//   yawRatio ≈  0     → 정면
//   yawRatio > +0.15  → 사용자 본인 기준 왼쪽으로 회전
//   yawRatio < -0.15  → 사용자 본인 기준 오른쪽으로 회전
export function computeYawRatio(landmarkPositions) {
  if (!landmarkPositions || landmarkPositions.length < 68) return 0;
  const left = landmarkPositions[0];
  const right = landmarkPositions[16];
  const nose = landmarkPositions[30];
  const span = right.x - left.x;
  if (span <= 0) return 0;
  return (nose.x - left.x) / span - 0.5;
}
